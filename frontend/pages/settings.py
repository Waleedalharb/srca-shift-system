...
def apply_theme():
    ...
    if theme == 'dark':
        ...
        <style>
            ...
            .stMarkdown, p, h1, h2, h3, h4, h5, h6, span, label, div { color: #FFFFFF !important; }
            .stSelectbox div[data-baseweb="select"] { background: #2D2D2D !important; border-color: #404040 !important; }
            .stSelectbox div[data-baseweb="select"] * { color: #FFFFFF !important; }
            div[data-baseweb="menu"] { background: #2D2D2D !important; border-color: #404040 !important; }
            div[data-baseweb="menu"] * { color: #FFFFFF !important; }
            .stTextInput input, .stNumberInput input, .stDateInput input { background: #2D2D2D !important; color: #FFFFFF !important; border-color: #404040 !important; }
            .stButton button { background: #0d6efd !important; color: white !important; border: none !important; }
            .stButton button:hover { background: #0b5ed7 !important; }
            div[data-testid="stVerticalBlock"] > div { background: #2D2D2D !important; border-color: #404040 !important; }
            .stTabs [data-baseweb="tab-list"] { background: #2D2D2D !important; }
            .stTabs [data-baseweb="tab"] { color: #FFFFFF !important; }
            .css-1d391kg, .css-163ttbj, .css-1wrcr25 { background: #252526 !important; }
            hr { border-color: #404040 !important; }
            .stAlert { background: #2D2D2D !important; color: #FFFFFF !important; border-color: #404040 !important; }
            .stDataFrame { background: #2D2D2D !important; }
            .stDataFrame * { color: #FFFFFF !important; }
            ::selection { background: #0d6efd !important; color: white !important; }
...
def show_settings():
        ...
        with st.form("profile_form"):
            ...
            with c2:
                ...
                dept = st.text_input("🏢 الإدارة / القسم",
                    value=user_data.get("dept", "قطاع الجنوب — الرياض"), disabled=True)
    ...
    with tabs[1]:
        ...
        with st.form("security_settings"):
            tfa = st.checkbox("🔐 تفعيل المصادقة الثنائية (2FA)",
                value=st.session_state.app_settings['tfa_enabled'])
            notif_login = st.checkbox("📧 إشعارات تسجيل الدخول على البريد",
                value=st.session_state.app_settings['login_notifications'])

            if st.form_submit_button("💾 حفظ إعدادات الأمان", use_container_width=True):
                st.session_state.app_settings['tfa_enabled'] = tfa
                st.session_state.app_settings['login_notifications'] = notif_login
                save_settings()
    ...
    with tabs[2]:
        with st.form("system_settings"):
            c1, c2 = st.columns(2)
            with c1:
                items_per_page = st.selectbox("📊 عناصر الصفحة",
                    [10, 20, 50, 100],
                    index=[10,20,50,100].index(st.session_state.app_settings['items_per_page']))
                
                refresh_rate_options = ["يدوياً", "كل دقيقة", "كل 5 دقائق", "كل 15 دقيقة"]
                refresh_rate = st.selectbox("🔄 تحديث البيانات",
                    refresh_rate_options,
                    index=refresh_rate_options.index(st.session_state.app_settings['refresh_rate']))
            with c2:
            ...
            if st.form_submit_button("💾 حفظ إعدادات النظام", use_container_width=True):
                st.session_state.app_settings['items_per_page'] = items_per_page
                st.session_state.app_settings['refresh_rate'] = refresh_rate
    ...
    with tabs[3]:
        with st.form("theme_settings"):
            ...
            font_size_options = list(font_options.keys())
            current_font = st.session_state.app_settings['font_size']
            new_font = st.select_slider(
                "حجم الخط",
                options=font_size_options,
                value=current_font if current_font in font_size_options else 'medium',
    ...
    # ══════════════════════════════════════════════════════
    with tabs[4]:
        theme_options_display = {'light': 'فاتح ☀️', 'dark': 'داكن 🌙'}
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;">
                ...
                <div style="background:#F8FAFC;border-radius:10px;padding:0.875rem;">
                    <p style="margin:0 0 0.3rem;font-size:0.75rem;color:#718096;">
                        الثيم الحالي</p>
                    <p style="margin:0;font-weight:700;color:#1A1A2E;">
                        {theme_options_display[st.session_state.app_settings['theme']]}
                    </p>
                </div>
            </div>
        </div>
        ...
        # إظهار الإعدادات الحالية (للتصحيح)
        with st.expander("📋 الإعدادات الحالية"):
